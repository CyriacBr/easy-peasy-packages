...
const commonUMD = config =>
  produce(config, draft => {
    ...
    draft.output.name = titleCase(pkgName.replace(/-/g, ' ')).replace(/ /g, '');
...
module.exports = [
  ...
  produce(commonUMD(baseConfig), draft => {
    draft.output.file = `dist/${pkgName}.umd.development.js`;
    draft.plugins = [
  ...
  produce(commonUMD(baseConfig), draft => {
    draft.output.file = `dist/${pkgName}.umd.js`;
    draft.plugins = [
  ...
  produce(baseConfig, draft => {
    ...
    draft.output.file = `dist/${pkgName}.esm.js`;
  }),
  // CJS build
  produce(baseConfig, draft => {
    draft.output.format = 'cjs';
    draft.output.file = `dist/${pkgName}.cjs.js`;
  }),
];
